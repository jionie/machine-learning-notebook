...
class RandomForest(object):
    ...
    def fitting(self):
        ...
        for i in range(self.num_trees):
            print("fitting tree: " + str(i+1) + " of " + str(self.num_trees) + " trees")
            start = time.time()
            train_data = self.bootstraps_datasets[i]
            train_label = self.bootstraps_labels[i]
            self.decision_trees[i].tree['features'] = np.arange(0, len(train_data[0]))
            self.decision_trees[i].tree['current_features'] = np.arange(0, len(train_data[0]))
            self.decision_trees[i].tree['depth'] = 0
            self.decision_trees[i].tree['min_gain'] = self.min_gain
            self.decision_trees[i].learn(train_data, train_label, self.max_depth)
            end = time.time()
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                dataset = self.bootstraps_datasets[0]
